...
- **Real-Time Components**: The `ChatWidget.tsx` component manages a direct WebSocket connection to the backend, rendering a responsive UI that dynamically updates based on granular agent events (e.g., token generation, tool execution).
- **Data Components**: `PortfolioWidget.tsx` and `OrderHistoryWidget.tsx` utilize `useEffect` to poll the REST endpoints, rendering financial data in styled CSS grid tables.
